...
2.0.0 重点优化了界面结构和 C++ 类型展开：

- 左侧栏拆成两个真正的同级面板：`1. stm32livewatch实时变量查看` 和 `2. 操作`。
- 实时变量很多时，变量面板自己滚动，下面的操作面板仍然保持可见。
- 常用操作放到变量面板标题栏：启动服务、停止服务、添加变量、刷新变量。
- 编辑器里选中 C/C++ 变量名后，可以右键直接添加到监视列表。
- 改进 C++ class/struct 的 DWARF 解析，支持 `PID pid_yaw;` 这类对象展开到 public 成员。
...
- 结构体和数组本身不直接读值，只展开子节点。

这样可以减少频繁刷新时的 OpenOCD 调用次数。

### 4. 自动刷新

扩展会按照 `stm32LiveWatch.refreshInterval` 定时刷新叶子变量。默认间隔是 `250ms`。只有值发生变化时才触发界面刷新，减少 VS Code UI 重绘压力。
...
- 结构体、class、数组本身不能直接写值，需要编辑它们的叶子成员。
...
1. 获取 `stm32-vscode-Live-watch-2.0.0.vsix`。
2. 打开 VS Code。
3. 按 `Ctrl+Shift+P`。
...
结构体、class、数组容器本身不能直接写入。请展开后修改叶子成员，例如：
